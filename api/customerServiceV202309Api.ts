...
export class CustomerServiceV202309Api {
     ...
     * @param needPlaintext Need message.plaintext
     * @param timeZone Time zone for formatting date and time in messages. The timestamps in messages will be returned in the specified time zone; other message content will not be affected.  The value must follow the IANA time zone format (for example, Asia/Shanghai, America/Los_Angeles). The default value is UTC.
     * @param shopCipher 
     */
    public async ConversationsConversationIdMessagesGet (conversationId: string, pageSize: number, xTtsAccessToken: string, contentType: string, pageToken?: string, locale?: string, sortOrder?: string, sortField?: string, needData?: boolean, needPlaintext?: boolean, timeZone?: string, shopCipher?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: CustomerService202309GetConversationMessagesResponse;  }> {
        ...
        if (needPlaintext !== undefined) {
            localVarQueryParameters['need_plaintext'] = ObjectSerializer.serialize(needPlaintext, "boolean");
        }

        if (timeZone !== undefined) {
            localVarQueryParameters['time_zone'] = ObjectSerializer.serialize(timeZone, "string");
        }
     ...
     * @param pageToken Paging cursor, this means where this query should start. For the next page, use \&quot;next_page_token\&quot; in response.
     * @param locale System message\&#39;s display language. The messages sent by System will be returned in the setting language; The messages sent by the buyer, the shop, the CS agent will not be affected. The default value is en (English).  Possible enumerations: - de-DE (German, Germany) - en (English) - en-GB (English, United Kingdom) - es-ES (Spanish, Spain) - es-MX (Spanish, Latin America) - fr-FR (French, France) - id-ID (Indonesian, Indonesia) - it-IT (Italian, Italy) - ja-JP (Japanese) - ms-MY (Malay, Malaysia) - th-TH (Thai, Thailand) - vi-VN (Vietnamese, Vietnam) - pt-BR (Portuguese, Brazil) - zh-CN (Simplified Chinese, China)
     * @param needSessionId Set to true when session id is required.
     * @param shopCipher 
     */
    public async ConversationsGet (pageSize: number, xTtsAccessToken: string, contentType: string, pageToken?: string, locale?: string, needSessionId?: boolean, shopCipher?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: CustomerService202309GetConversationsResponse;  }> {
        ...
        if (needSessionId !== undefined) {
            localVarQueryParameters['need_session_id'] = ObjectSerializer.serialize(needSessionId, "boolean");
        }
